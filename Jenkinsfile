            ...
            steps {
                ...
                withCredentials([ usernamePassword ( 
                    credentialsId: 'veracode_login', usernameVariable: 'VERACODE_API_ID', passwordVariable: 'VERACODE_API_KEY') ]) {
                        veracode applicationName: "${VERACODE_APP_NAME}", sandboxName: "${VERACODE_SANDBOX_NAME}", criticality: 'VeryHigh', deleteIncompleteScanLevel: '1', debug: true, scanName: "${BUILD_TAG}-${env.HOST_OS}", uploadIncludesPattern: 'app/target/verademo.war', vid: "${VERACODE_API_ID}", vkey: "${VERACODE_API_KEY}"
                    }
            }
        }
